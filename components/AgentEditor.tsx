...
const SliderInput: React.FC<{ label: string, value: number, min: number, max: number, step: number, onChange: (val: number) => void }> = ({ label, value, min, max, step, onChange }) => (
  <div>
    <div className="flex justify-between items-center mb-1">
      ...
      <input type="number" value={value} onChange={(e) => onChange(parseFloat(e.target.value))} className="w-16 bg-zinc-900 border border-zinc-800 rounded px-2 py-0.5 text-xs text-indigo-400 font-mono focus:outline-none" />
    </div>
    <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} className="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
  </div>
);
...
export const AgentEditor: React.FC<AgentEditorProps> = ({ agent, tools, onSave, onTest, onCancel, onSaveAndCompare }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#09090b]">
      {/* Header */}
      <header className="p-4 border-b border-zinc-800 flex justify-between items-center bg-[#0c0c0e] sticky top-0 z-20">
        <div className="flex items-center gap-4">
          <button onClick={onCancel} className="p-2 text-zinc-500 hover:text-zinc-100"><X className="w-5 h-5" /></button>
          <div className="w-px h-6 bg-zinc-800" />
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-indigo-400" />
            <h2 className="text-lg font-bold text-zinc-100 truncate">{formData.name || "New Agent"}</h2>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative" ref={versionDropdownRef}>
            <button onClick={() => setIsVersionDropdownOpen(p => !p)} className="flex items-center gap-2 px-4 py-2 bg-zinc-800/50 border border-zinc-700 rounded-lg text-xs font-bold text-zinc-300 hover:bg-zinc-800">
              <GitCommit className="w-3.5 h-3.5" /> Version {formData.version} <ChevronDown className="w-3.5 h-3.5" />
            </button>
            {isVersionDropdownOpen && (
              <div className="absolute top-full right-0 mt-2 w-72 bg-[#18181b] border border-zinc-800 rounded-xl shadow-2xl p-2 z-30 animate-in fade-in zoom-in-95">
                ...
                <div className="max-h-60 overflow-y-auto scrollbar-thin pr-1">
                  {allVersions.map(v => (
                    <div key={v.version} className="flex items-center justify-between group hover:bg-zinc-800/50 rounded-md">
                      <button onClick={() => handleVersionLoad(v)} className="flex-1 text-left flex justify-between items-center p-2 rounded-md text-xs">
                        <span>Version {v.version} {v.version === formData.version && <span className="text-indigo-400">(Current)</span>}</span>
                        <span className="text-zinc-500">{new Date(v.createdAt).toLocaleDateString()}</span>
                      </button>
                       {v.version !== formData.version && (
                         <button onClick={() => setVersionToDelete(v.version)} className="p-2 text-zinc-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => onSaveAndCompare(formData)} className="w-full flex items-center gap-2 p-2 rounded-md hover:bg-zinc-700/80 text-xs">
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-[380px] border-r border-zinc-800 p-6 space-y-8 overflow-y-auto scrollbar-thin">
          <section className="space-y-4">
            ...
            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Agent Name</label><input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Agent Description</label><textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={4} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Domain</label><select value={formData.domain} onChange={(e) => setFormData({ ...formData, domain: e.target.value })} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 appearance-none"><option value="">Select Domain...</option>{domains.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
          </section>
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <select value={formData.config.aiEngine} onChange={(e) => setFormData({ ...formData, config: { ...formData.config, aiEngine: e.target.value } })} className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-xs focus:outline-none">{engines.map(eng => <option key={eng.id} value={eng.name}>{eng.name}</option>)}</select>
              <select value={formData.config.model} onChange={(e) => setFormData({ ...formData, config: { ...formData.config, model: e.target.value } })} className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-xs focus:outline-none">{filteredModels.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select>
            </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto scrollbar-thin space-y-6">
          ...
          <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Role</label><textarea value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} rows={1} placeholder="e.g., Senior Software Architect" className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Goal</label><textarea value={formData.goal} onChange={(e) => setFormData({ ...formData, goal: e.target.value })} rows={5} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500 w-full" /></div>
            <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Backstory</label><textarea value={formData.backstory} onChange={(e) => setFormData({ ...formData, backstory: e.target.value })} rows={5} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500 w-full" /></div>
          </div>
          <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Instructions</label><textarea value={formData.taskDescription} onChange={(e) => setFormData({ ...formData, taskDescription: e.target.value })} rows={16} placeholder="Detailed instructions..." className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-4 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500 mono" /></div>
          <div className="space-y-2">
            <div className="flex items-center justify-between"><label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Execution Inputs</label><button onClick={addInput} className="text-xs font-bold text-indigo-400 hover:text-indigo-300"><Plus className="w-3 h-3 inline-block mr-1" />Add</button></div>
            {formData.inputs.map((input, idx) => (
              <div key={idx} className="flex gap-2 items-center"><textarea value={input.description} onChange={(e) => updateInput(idx, 'description', e.target.value)} rows={1} placeholder="Description" className="flex-1 bg-zinc-950 border border-zinc-800 rounded px-2 py-1 text-xs focus:ring-1 focus:ring-indigo-500 outline-none resize-y" /><input type="text" value={input.parameter} onChange={(e) => updateInput(idx, 'parameter', e.target.value.replace(/\s+/g, '_'))} placeholder="param_name" className="w-40 bg-zinc-950 border border-zinc-800 rounded px-2 py-1 text-xs mono text-indigo-400 focus:ring-1 focus:ring-indigo-500 outline-none" /><button onClick={() => setInputToDelete(idx)} className="p-1 text-zinc-600 hover:text-red-400"><Trash2 className="w-3 h-3" /></button></div>
            ))}
          </div>
          <div><label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Expected Output</label><textarea value={formData.expectedOutput} onChange={(e) => setFormData({ ...formData, expectedOutput: e.target.value })} rows={8} className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-sm resize-y focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
          <div>
            <label className="text-[10px] font-bold text-zinc-500 uppercase mb-1 block">Output File Extension</label>
            <div className="flex gap-2">
              <select value={isOtherExtension ? 'other' : formData.outputFileExtension} onChange={e => handleExtensionChange(e.target.value)} className="w-48 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 appearance-none">
                <option value="" disabled>Select...</option>
                {fileExtensions.map(fe => <option key={fe.id} value={fe.extension}>{fe.name} ({fe.extension})</option>)}
                <option value="other">Other...</option>
              </select>
              {isOtherExtension && <input type="text" value={customExtension} onChange={e => handleCustomExtensionChange(e.target.value)} placeholder=".custom" className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />}
            </div>
        ...
        <div className={`border-l border-zinc-800 p-6 space-y-4 overflow-y-auto scrollbar-thin transition-all duration-300 ${isToolsCollapsed ? 'w-16' : 'w-[420px]'}`}>
          ...
          {!isToolsCollapsed && (<>
            <div className="relative"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-500" /><input type="text" value={toolSearch} onChange={(e) => setToolSearch(e.target.value)} placeholder="Search tools..." className="w-full bg-zinc-900 border border-zinc-800 rounded-lg pl-8 pr-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-indigo-500" /></div>
            <div className="space-y-2">
              {filteredTools.map(tool => (
                <button key={tool.id} onClick={() => toggleTool(tool.id)} className={`w-full p-3 rounded-lg border text-left transition-all relative ${formData.toolIds?.includes(tool.id) ? 'bg-indigo-600/10 border-indigo-600/40' : 'bg-zinc-900 border-zinc-800 hover:border-zinc-700'}`}>
                  <h4 className="font-bold text-xs text-zinc-100">{tool.name}</h4>
      ...
      {inputToDelete !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0c0c0e] border border-zinc-800 p-6 rounded-2xl max-w-sm w-full shadow-2xl">
            <h3 className="text-sm font-bold text-zinc-100 mb-2">Confirm Deletion</h3>
            <p className="text-xs text-zinc-400 mb-6">Are you sure you want to delete the input parameter "{formData.inputs[inputToDelete]?.parameter || 'untitled'}"?</p>
            <div className="flex gap-3"><button onClick={() => setInputToDelete(null)} className="flex-1 px-3 py-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 rounded-lg font-bold text-xs border border-zinc-800">Cancel</button><button onClick={() => removeInput(inputToDelete)} className="flex-1 px-3 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg font-bold text-xs">Delete</button></div>
          </div>
        </div>
      )}
      {versionToDelete !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0c0c0e] border border-zinc-800 p-6 rounded-2xl max-w-sm w-full shadow-2xl">
            <h3 className="text-sm font-bold text-zinc-100 mb-2">Confirm Version Deletion</h3>
            <p className="text-xs text-zinc-400 mb-6">Are you sure you want to permanently delete Version {versionToDelete}? This action cannot be undone.</p>
            <div className="flex gap-3"><button onClick={() => setVersionToDelete(null)} className="flex-1 px-3 py-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 rounded-lg font-bold text-xs border border-zinc-800">Cancel</button><button onClick={handleConfirmDeleteVersion} className="flex-1 px-3 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg font-bold text-xs">Delete</button></div>
          </div>
        </div>
      )}
    </div>
