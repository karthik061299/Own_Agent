...
import { 
  ...
  Search, Clock, Box, Download, ChevronDown, ChevronUp, Layers, FileText, Square, Hammer, AlertCircle, Cpu, Eye, EyeOff, ThumbsUp, ThumbsDown, MessageSquare, GitCommit, Upload, X, PanelLeftClose, PanelRight
} from 'lucide-react';
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-2">
          {log.nodeId === 'manager' || log.nodeId === 'system' ? (
             <div>
              <span className={`${log.nodeId === 'manager' ? 'text-amber-600 dark:text-amber-500' : 'text-zinc-600 dark:text-zinc-500'} font-bold uppercase tracking-tighter`}>{log.agentName} &gt; </span>
              <span className={`${log.nodeId === 'manager' ? 'text-amber-900/90 dark:text-amber-100/90' : 'text-zinc-500 dark:text-zinc-400'} italic`}>{log.output}</span>
            </div>
          ) : (
            <>
              <div>
              ...
              <div className="max-h-0 overflow-hidden group-hover:max-h-none transition-all duration-500 ease-in-out">
                <div className="pt-3 mt-2 border-t border-zinc-200 dark:border-zinc-800/60 space-y-4">
...
// FIX: Changed response.text() to response.text as per @google/genai guidelines. The 'text' property directly returns the string output.
        ...
        <div className="flex border-r border-zinc-200 dark:border-zinc-800">
          ...
          {isHistoryPanelCollapsed ? (
            ...
            <div className="w-[260px] flex flex-col bg-zinc-100/10 dark:bg-[#0c0c0e]/10 overflow-hidden relative">
              ...
              <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100/20 dark:bg-zinc-900/20">
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500" />
                  <input type="text" placeholder="Search history..." value={executionSearch} onChange={(e) => setExecutionSearch(e.target.value)} className="w-full bg-zinc-200/50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-lg pl-9 pr-3 py-1.5 text-[10px] outline-none focus:ring-1 focus:ring-indigo-500" />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
                {filteredExecutions.map(ex => (<button key={ex.id} onClick={() => setActiveExecutionId(ex.id)} className={`w-full text-left p-4 rounded-xl border transition-all relative overflow-hidden group ${activeExecutionId === ex.id ? 'bg-indigo-600/10 border-indigo-600/40 shadow-lg' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 shadow-sm'}`}><div className="flex justify-between items-start mb-2"><span className="text-[11px] font-bold text-zinc-800 dark:text-zinc-200 truncate pr-2">{ex.workflow_name}</span><div className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)] ${ex.status === 'completed' ? 'bg-emerald-500' : ex.status === 'paused' ? 'bg-yellow-500' : ex.status === 'stopped' ? 'bg-amber-500' : ex.status === 'running' ? 'bg-indigo-500 animate-pulse' : 'bg-red-500'}`} /></div><div className="flex items-center justify-between text-[9px] text-zinc-500 dark:text-zinc-600"><div className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> {new Date(ex.timestamp).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</div>{ex.duration !== undefined && ex.duration !== null && (<div className="text-indigo-500 dark:text-indigo-400 font-bold">{formatDuration(ex.duration)}</div>)}</div></button>))}
           ...
           {activeExecutionId ? (<div className="flex-1 flex flex-col overflow-y-auto scrollbar-thin p-8"><div className="max-w-4xl mx-auto w-full space-y-8 animate-in fade-in duration-300">
            {isPausedForHumanInput && humanCheckpointData && (
              <section className="bg-yellow-500/10 border-2 border-dashed border-yellow-500/30 p-8 rounded-3xl space-y-6 animate-in zoom-in-95 duration-300">
                <div className="flex justify-between items-center"><h3 className="text-xl font-bold text-yellow-600 dark:text-yellow-400 flex items-center gap-3"><ThumbsUp className="w-6 h-6"/> Human Input Required</h3><div className="flex items-center gap-2"><div className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">{humanCheckpointData.agent.name}</div><div className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"/></div></div>
